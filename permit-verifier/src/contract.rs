...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> Result<Binary, ContractError> {
    let res = match msg {
        ...
        QueryMsg::WithPermit {
            ...
            permit,
            query,
        } => permit_queries(deps, address_hrp, permit, query),
...
fn permit_queries(
    ...
    let address_hrp = address_hrp.unwrap_or_else(|| "secret".to_string());
    let viewer = validate(
        deps,
        PREFIX_REVOKED_PERMITS,
        &permit,
...
fn query_permissioned(_deps: Deps, viewer: String) -> Result<Binary, ContractError> {
    Ok(to_binary(&QueryAnswer::PermitExAns {
        message: format!("This permit is signed by {}", viewer),
    })?)
}
